...
### 4.1 Contrastive Language-Image Pre-Training (CLIP)
...
### 4.2 Rotation-oriented Continuous Image Translation (Ro-CIT)
...
### 4.3 U-Net 전이 학습 (Transfer Learning)
...
- 주요 라이브러리:
  - `torch`, `torchvision`
  - `transformers` (CLIP, BERT, DiT 등)
  - `tqdm`, `numpy`, `pandas`
  - `matplotlib`, `seaborn`
  - `scikit-learn` (metrics, PCA, t-SNE/UMAP 등)
...
3. **학습**
   ...
   - rotation-oriented augmentation 적용
